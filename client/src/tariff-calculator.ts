...
export type Product =
  ...
  | "LAP_Green"
  | "EducationLoan"
  | "EducationLoan_Unsecured"
  | "EducationLoan_Secured";
...
export function calcProcessingFee(inputs: UserInputs): FeeRow {
  ...
  if (product === "PersonalLoan") {
    ...
    amount = clamp(calculated, config.min, config.max);
    basis = `${(config.rate * 100).toFixed(2)}% (Min ${formatCurrency(config.min)}, Max ${formatCurrency(config.max)})`;
  } else if (product.startsWith("EducationLoan")) {
    // Education Loan: Unsecured uses Personal Loan fees, Secured uses LAP fees
    if (product === "EducationLoan_Unsecured") {
      // Use Personal Loan tiered fees (12.01)
      const tier = PL_PROCESSING.find((t) => loanAmount <= t.max);
      amount = tier?.fee || PL_PROCESSING[PL_PROCESSING.length - 1].fee;
      label = "Processing Fee (Education Loan - Unsecured)";
      basis = "Tiered flat fee (same as Personal Loan)";
    } else if (product === "EducationLoan_Secured") {
      // Use LAP percentage fees (12.07)
      const config = LAP_STANDARD;
      const calculated = roundToLKR(loanAmount * config.rate);
      amount = clamp(calculated, config.min, config.max);
      label = "Processing Fee (Education Loan - Secured)";
      basis = `${(config.rate * 100).toFixed(2)}% (Min ${formatCurrency(config.min)}, Max ${formatCurrency(config.max)}) - same as LAP`;
    } else {
      // Default EducationLoan (for backward compatibility, treat as unsecured)
      const tier = PL_PROCESSING.find((t) => loanAmount <= t.max);
      amount = tier?.fee || PL_PROCESSING[PL_PROCESSING.length - 1].fee;
      label = "Processing Fee (Education Loan)";
      basis = "Tiered flat fee";
    }
  }
...
export function calcApplicationFee(inputs: UserInputs): number {
  ...
  // 12.03 - Only for Housing/LAP/Education(Secured) at lodgement
  const isHousingOrLAP = product.startsWith("HousingLoan") || product.startsWith("LAP");
  const isEducationSecured = product === "EducationLoan_Secured";
  return (isHousingOrLAP || isEducationSecured) ? APPLICATION_FEE : 0;
}
...
export function calcLegal(inputs: UserInputs): FeeRow[] {
  ...
  // Only for Housing/LAP/Education(Secured) products
  const isHousingOrLAP = product.startsWith("HousingLoan") || product.startsWith("LAP");
  const isEducationSecured = product === "EducationLoan_Secured";
  if (!isHousingOrLAP && !isEducationSecured) return rows;
