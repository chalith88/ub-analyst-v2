...
/** Close any lity lightbox popups/modals that might be blocking interactions */
async function closeLityPopups(page: Page) {
  try {
    // Close lity lightbox by clicking the close button or overlay
    await page.evaluate(() => {
      // Remove lity overlays and dialogs
      document.querySelectorAll('.lity, .lity-wrap, [role="dialog"][aria-label*="Dialog"]').forEach(el => {
        (el as HTMLElement).remove();
      });
      // Click close buttons if they exist
      document.querySelectorAll('[data-lity-close], .lity-close, button[aria-label*="close" i]').forEach(btn => {
...
export async function scrapeBOC(opts: Opts = {}): Promise<RateRow[]> {
  ...
  try {
    ...
    await closeLityPopups(page);
    await page.waitForTimeout(300);
    
    // Ensure Advance Rates tab is selected/active
    const tabBtn = await page.$('a[href="#advance-rates"]');
    if (tabBtn) {
      await closeLityPopups(page); // Close popups again before clicking
      await tabBtn.click({ force: true }).catch(async () => {
        // If click fails, try JavaScript click
        await page.evaluate(() => {
          const btn = document.querySelector('a[href="#advance-rates"]') as HTMLElement;
          if (btn) btn.click();
        });
      });
    }
      ...
      /* --------------------------- GREEN LOANS (SOLAR) -------------------- */
        ...
        if (rows) {
          for (const t of rows) {
            ...
            const amountLabel = t[0] || "";
            const rateText = t[1] || "";
            const rate = pct(rateText);
            
            if (!rate || !amountLabel) continue;

            // Green loans are typically short-term (5 years max)
            const base: Omit<RateRow, "tenureYears"> = {
              bank: BANK,
              product: "Solar Loan",
              type: "Fixed",
              ...
              source: SRC,
              updatedAt,
              notes: `Green Loans - ${amountLabel.replace(/\(Residential\)/i, "").trim()}`,
            };
            fanOutPush(out, base, "Up to 5 Years", 5);
          }
        }
      }

      /* ------------------- GOVERNMENT PENSIONERS' LOAN -------------------- */
      else if (title.includes("pensioner")) {
        const tbody = await getTBody(wrapper);
        if (!tbody) continue;
        ...
        if (rows) {
          ...
          for (const t of rows) {
            ...
            const tenureText = t[0] || "";
            const rateText = t[1] || "";
            const rate = pct(rateText);
            
            if (!rate) continue;

            // Extract English portion from multilingual text (Sinhala/Tamil/English)
            const englishMatch = tenureText.match(/Up to (\d+) Years?/i);
            if (!englishMatch) continue;
            
            const upToYear = Number(englishMatch[1]);
            const fromYear = lastMaxYear + 1;
            const toYear = upToYear;
            
            const tenureLabel = `Up to ${upToYear} Years`;
            
            // Fan out from (lastMax + 1) to upToYear
            for (let y = fromYear; y <= toYear; y++) {
              out.push({
                ...
                source: SRC,
                updatedAt,
                notes: "Government Pensioners' Loan Scheme",
                tenureYears: y,
              });
            }
