...
export async function scrapeCombankTariff(opts: { show?: boolean } = {}): Promise<FeeRow[]> {
  const browser = await chromium.launch({ 
    headless: !opts.show,
    slowMo: opts.show ? 200 : 0,
  });
...
if (require.main === module) {
  console.log("Starting Commercial Bank tariff scraper...");
  scrapeCombankTariff({ show: false })
    .then(rows => {
      console.log(`✅ Scraped ${rows.length} tariff rows from Commercial Bank`);
      console.log(JSON.stringify(rows, null, 2));
    })
    .catch(err => {
      console.error("❌ Error scraping Commercial Bank tariff:", err);
      process.exit(1);
    });
}
