...
/**
 * Scrape People's Bank Pensioner Loan (Parinatha Vishrama Loan) from quick & easy loans page.
 * Static data based on website information:
 * - Interest Rate: 11.5% - 13.0% (Variable rates available)
 * - Max Tenure: 15 years
 * - No Upper Limit on loan amount
 * URL: https://www.peoplesbank.lk/quick-easy-loans-a-c-en/
 */
export async function scrapePeoplesPensionerLoan(): Promise<RateRow[]> {
  const out: RateRow[] = [];
  const now = new Date().toISOString();
  const minRate = "11.50%";
  const maxRate = "13.00%";
  const source = "https://www.peoplesbank.lk/quick-easy-loans-a-c-en/";
  
  // Generate 15 rows (1-15 years) with min/max rates
  for (let year = 1; year <= 15; year++) {
    out.push({
      ...
      source,
      updatedAt: now,
      notes: "Parinatha Vishrama Loan - Minimum rate (No Upper Limit)",
    });
    out.push({
      bank: "People's Bank",
      product: "Pensioner Loan",
      ...
      source,
      updatedAt: now,
      notes: "Parinatha Vishrama Loan - Maximum rate (No Upper Limit)",
    });
  }
