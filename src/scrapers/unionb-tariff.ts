...
async function loadTariffLines(): Promise<string[]> {
  const res = await fetch(SOURCE);
  if (!res.ok) {
    throw new Error(`Failed to download Union Bank tariff PDF (${res.status} ${res.statusText})`);
  }
  ...
  for (let pageNo = 1; pageNo <= pdf.numPages; pageNo++) {
    ...
    for (const item of textItems) {
      const str = (item as any).str as string | undefined;
      if (!str) continue;

      current.push(str);
      if (str.endsWith(" ") || str.length > 35) {
        const joined = clean(current.join(" "));
...
export async function scrapeUnionbTariff(): Promise<FeeRow[]> {
  ...
  const baseLines = await loadTariffLines();
  ...
  // Add Education Loan fees based on mappings:
  // - Unsecured = same as Personal Loan
  // - Secured = same as Loan Against Property
  const educationRows: FeeRow[] = [];
  
  // Clone Personal Loan fees for Education Loan (Unsecured)
  result
    .filter(row => row.product === "Personal Loan")
    .forEach(row => {
      educationRows.push({
        ...row,
        product: "Education Loan",
        feeCategory: row.feeCategory.replace("Personal Loan", "Education Loan (Unsecured)"),
        notes: row.notes ? `${row.notes} | Unsecured` : "Unsecured"
      });
    });
  
  // Clone Loan Against Property fees for Education Loan (Secured)
  result
    .filter(row => row.product === "Loan Against Property")
    .forEach(row => {
      educationRows.push({
        ...row,
        product: "Education Loan",
        feeCategory: row.feeCategory.replace("Loan against Property", "Education Loan (Secured)").replace("Loan Against Property", "Education Loan (Secured)"),
        notes: row.notes ? `${row.notes} | Secured` : "Secured"
      });
    });

  result = [...result, ...educationRows];
