...
function normalizeProduct(label: string): "Home Loan" | "Education Loan" | "Solar Loan" | null {
  if (/Home\s*Financing/i.test(label)) return "Home Loan";
  if (/Education\s*Financing/i.test(label)) return "Education Loan";
  if (/Solar\s*Financing/i.test(label)) return "Solar Loan";
  return null;
}
...
export async function scrapeAmana(): Promise<RateRow[]> {
  ...
  const collected: { [k in "Home Loan" | "Education Loan" | "Solar Loan"]?: { min?: string; max?: string } } = {};
  ...
  for (const want of ["Home Loan", "Education Loan", "Solar Loan"] as const) {
  ...
  if (collected["Solar Loan"]) {
    const { min, max } = collected["Solar Loan"]!;
    const baseRows = buildMinMaxRows("Solar Loan", min, max);
    for (const r of baseRows) fanOutYears(out, r, 10); // 1..10 years (typical solar loan tenure)
  }
