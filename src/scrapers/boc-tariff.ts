...
export type TariffRow = {
  bank: string;
  product: "Home Loan" | "LAP" | "Personal Loan" | "Education Loan" | "Pensioner Loan";
  feeCategory: string;
  description?: string;
  amount: string;
  updatedAt: string;
    ...
    for (const h3 of headers) {
      ...
      for (const cells of tableRows) {
        ...
        if (
          ...
          const fanProducts: TariffRow["product"][] = ["Home Loan", "LAP", "Personal Loan", "Pensioner Loan", "Education Loan"];
        ...
        // Duplicate Personal Loan tariffs for Pensioner Loan (BOC uses same tariff for both)
        if (product === "Personal Loan") {
          rows.push({
            ...
            updatedAt: nowISO(),
            source: `${SEED}#general-tariff`,
            notes: "Same tariff as Personal Loan"
          });
        }
      }
