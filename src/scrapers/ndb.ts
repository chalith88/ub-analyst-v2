...
async function scrapeSolarLoan(table: Locator, nowISO: string): Promise<RateRow[]> {
  const out: RateRow[] = [];
  const { idxDesc, idxMin, idxMax, idxOthers } = await mapColumns(table);
  ...
  for (const tr of trs) {
    ...
    // Match "Solar Vantage - Personal Loans"
    if (!/solar\s*vantage/i.test(descCell)) continue;

    const minCell = idxMin != null ? cells[idxMin] ?? "" : "";
    const maxCell = idxMax != null ? cells[idxMax] ?? "" : "";
    const othCell = idxOthers != null ? cells[idxOthers] ?? "" : "";

    const minRate = asRate(minCell);
    const maxRate = asRate(maxCell);
    const othRate = asRate(othCell);

    // Create entries for each available rate column
    const pushes: Array<{ label: string; val?: string }> = [
      { label: "Min rate", val: minRate },
      { label: "Max rate", val: maxRate },
      { label: "Others", val: othRate },
    ];
    ...
    for (const { label, val } of pushes) {
      ...
      out.push({
        ...
        source: URL,
        updatedAt: nowISO,
        notes: `Solar Vantage - Personal Loans · ${label}`,
      });
    }
  }
...
async function scrapePensionerLoan(table: Locator, nowISO: string): Promise<RateRow[]> {
  const out: RateRow[] = [];
  const { idxDesc, idxMin, idxMax } = await mapColumns(table);
  ...
  for (const tr of trs) {
    ...
    // Match "Aachara Loans 1-15 Years"
    if (!/aachara\s*loans?/i.test(descCell)) continue;
    ...
    // Use whichever rate is available
    const rate = minRate || maxRate;
    if (!rate) continue;

    // Parse "1-15 Years" from description
    const tenureMatch = descCell.match(/(\d{1,2})\s*-\s*(\d{1,2})\s*years?/i);
    let years: number[] = [];
    if (tenureMatch) {
      const start = parseInt(tenureMatch[1], 10);
      const end = parseInt(tenureMatch[2], 10);
      years = Array.from({ length: end - start + 1 }, (_, i) => start + i);
    }

    const rows = fanOutByYears<RateRow>(
      {
        bank: "NDB Bank",
        product: "Pensioner Loan",
        type: "Fixed",
        tenureLabel: tenureMatch ? `${tenureMatch[1]}-${tenureMatch[2]} Years` : undefined,
        rateWithSalary: rate,
        rateWithoutSalary: undefined,
        source: URL,
        updatedAt: nowISO,
...
export async function scrapeNDB(opts?: { show?: boolean; slow?: number }): Promise<RateRow[]> {
  ...
  try {
    ...
    rows.push(...(await scrapeSolarLoan(table, now)));
    rows.push(...(await scrapePensionerLoan(table, now)));
  } finally {
    await browser.close();
  }
