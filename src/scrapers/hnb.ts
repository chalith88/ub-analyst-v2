...
export async function scrapeHNB(opts?: { show?: boolean; slow?: number }): Promise<RateRow[]> {
  ...
  try {
    ...
    // PERSONAL LOANS (base + Special + LAP + Solar)
    ...
    rows.push(...await scrapeSolarLoan(page, now));

    // EDUCATION LOANS
    await clickLeftMenu(page, "Education Loans");
    rows.push(...await scrapeEducation(page, now));

    // PENSIONER LOANS (Sathkara Loans)
    await clickLeftMenu(page, "Sathkara Loans");
    rows.push(...await scrapePensionerLoan(page, now));
...
// --- SOLAR LOAN (Roof Top Solar) --------------------------------------------

async function scrapeSolarLoan(page: Page, now: string): Promise<RateRow[]> {
  const out: RateRow[] = [];
  const source = URL;
  ...
  for (const tbl of tables) {
    ...
    // Look for "Solar" or "Roof Top Solar" headings
    if (!/roof\s*top\s*solar|solar\s*loan|personal.*solar/i.test(h)) continue;
    
    const headers = (await tbl.locator("thead tr th").allTextContents()).map((s) => clean(s).toLowerCase());
    const bodyRows = await tbl.locator("tbody tr").all();

    // Find column indices for Period and Interest Rate
    const idxPeriod = headers.findIndex((h) => /period|tenor|tenure/i.test(h));
    const idxRate = headers.findIndex((h) => /interest.*rate|rate/i.test(h));

    for (const tr of bodyRows) {
      const tds = (await tr.locator("td").allTextContents()).map(clean);
      if (!tds.length) continue;

      // Get period and rate from columns (with fallbacks)
      const periodCell = tds[idxPeriod >= 0 ? idxPeriod : 0] || "";
      const rateCell = tds[idxRate >= 0 ? idxRate : 1] || "";

      // Extract years from period (e.g., "3 Years" → [3])
      let years = expandTenureYears(periodCell);
      
      // Fallback: try to parse single year number
      if (!years.length) {
        const m = periodCell.match(/\b(\d{1,2})\s*(?:year|yr)s?\b/i);
        if (m) {
          years = [parseInt(m[1], 10)];
        }
      }

      // Parse rate (e.g., "11.50%")
      const rate = asPercent(rateCell) || undefined;

      if (rate && years.length) {
        out.push(
          ...fanOutByYears<RateRow>({
            bank: "HNB",
            product: "Solar Loan",
            type: "Fixed",
            tenureLabel: periodCell,
            rateWithSalary: rate,
            rateWithoutSalary: rate,
            source,
            updatedAt: now,
            notes: "Roof Top Solar"
          }, years)
        );
      }
    }
...
// --- PENSIONER LOANS (Sathkara Loans to Government Pensioners) -------------

async function scrapePensionerLoan(page: Page, now: string): Promise<RateRow[]> {
  const out: RateRow[] = [];
  const source = URL;
  ...
  for (const tbl of tables) {
    ...
    // Look for "Sathkara" or "Pensioner" headings
    if (!/sathkara|pensioner/i.test(h)) continue;
    
    const headers = (await tbl.locator("thead tr th").allTextContents()).map((s) => clean(s).toLowerCase());
    const bodyRows = await tbl.locator("tbody tr").all();

    // Find column indices for Period and Rate
    const idxPeriod = headers.findIndex((h) => /period|tenor|tenure/i.test(h));
    const idxRate = headers.findIndex((h) => /salaried.*rate|rate/i.test(h));
    ...
    for (const tr of bodyRows) {
      ...
      // Extract years from period (e.g., "Up to 3 years" → [1,2,3])
      let years = expandTenureYears(periodCell);
      
      // Fallback: try to parse year ranges or single years
      if (!years.length) {
        const m = periodCell.match(/\b(\d{1,2})\s*[-–]\s*(\d{1,2})\s*years?/i);
        if (m) {
          const a = parseInt(m[1], 10), b = parseInt(m[2], 10);
          const from = Math.min(a, b), to = Math.max(a, b);
          years = Array.from({ length: to - from + 1 }, (_, i) => from + i);
        } else {
          const single = periodCell.match(/\b(\d{1,2})\s*(?:years?|yrs?)\b/i);
          if (single) {
            years = [parseInt(single[1], 10)];
          }
        }
      }

      // Parse rate (e.g., "12.00%")
      const rate = asPercent(rateCell) || undefined;

      // Determine if it's Fixed or includes Floating option
      const typeRaw = decideType(rateCell, "", h);
      ...
      if (rate && years.length) {
        out.push(
          ...fanOutByYears<RateRow>({
            ...
            source,
            updatedAt: now,
            notes: "Sathkara Loans to Government Pensioners"
          }, years)
        );
      }
    }
