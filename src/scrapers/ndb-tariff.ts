...
function parsePensioner(rows: Row[], now: string): FeeRow[] {
  const out: FeeRow[] = [];
  const pensionerRows = rows.filter((r) => r.page === 23);
  
  // Find section 11.14 AACHARA Loans Processing Fees
  const processingStart = pensionerRows.findIndex((r) => 
    /11\.14\s+AACHARA.*Processing Fees/i.test(r.text)
  );
  
  if (processingStart >= 0) {
    // Look for the processing fee rows
    for (let i = processingStart + 1; i < pensionerRows.length; i++) {
      const row = pensionerRows[i];
      
      // Stop at next section
      if (/11\.15|11\.16|11\.17/i.test(row.text)) break;
      
      const text = row.text;
      
      // New Loans processing fee
      if (/^New Loans\s+LKR/i.test(text)) {
      ...
      // Top Up Loans processing fee
      if (/^Top Up Loans/i.test(text)) {
        const match = text.match(/LKR\s*([\d,]+)/);
        if (match) {
          out.push(makeRow(
            "Pensioner Loan",
      ...
      // Settling with existing loan processing fee
      if (/^Settling with.*existing loan/i.test(text)) {
        const match = text.match(/LKR\s*([\d,]+)/);
        if (match) {
          out.push(makeRow(
            "Pensioner Loan",
            "Processing fee",
            "Settling with existing loan",
            `LKR ${match[1]}`,
            now
          ));
        }
      ...
      // Early settlements - Part settlement
      if (/Part settlement/i.test(text) && /%/.test(text)) {
      ...
      // Early settlements - Full settlement
      if (/Full settlement/i.test(text) && /%/.test(text)) {
        const match = text.match(/(\d+%[^%]*)/);
        if (match) {
          out.push(makeRow(
            "Pensioner Loan",
...
export async function scrapeNdbTariff(opts?: { show?: boolean; slow?: number }): Promise<FeeRow[]> {
  ...
  try {
    ...
    out.push(...parsePensioner(rows, now));
