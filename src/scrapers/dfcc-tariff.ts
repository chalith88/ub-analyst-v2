...
/* ---------------- Constants ---------------- */
const PDF_URL =
  "https://properties.dfcc.lk/dfccweb/uploads/7d073e00-f294-409f-ab28-2bc3b559b7b2/DFCC-Bank-PLC-Tariff-2025-Version-3.02.pdf";
...
/* ---------------- Solar Loan (5.2.1.7) ---------------- */
function parseSolarLoanSlabs(lines: string[], source: string): FeeRow[] {
  const out: FeeRow[] = [];
  const sectionIdx = lines.findIndex((l) =>
    l.trim().startsWith("5.2.1.7Solar Loan") || l.trim().startsWith("5.2.1.7 Solar Loan")
  );
  if (sectionIdx < 0) return out;
  
  const prefix = "5.2.1.7.";
  const seen = new Set<string>();
  ...
  for (let i = 1; i <= 5; ++i) {
    ...
    const key = `Solar Loan|${description}|${amount}`;
    if (seen.has(key)) continue;
    seen.add(key);
...
/* ---------------- Garusaru Pensioner Loan (5.2.1.4) ---------------- */
function parseGarusaruPensionerLoan(lines: string[], source: string): FeeRow[] {
  const out: FeeRow[] = [];
  
  // Look for "5.2.1.4.1" which should be "Irrespective of the loan amount"
  // OCR gives us: "5.2.1.4.1Irrespective of the loan amountN/A2,500"
  const slabLine = lines.find((l) => l.trim().startsWith("5.2.1.4.1"));
  if (!slabLine) return out;

  // Remove section number
  let text = slabLine.replace(/^5\.2\.1\.4\.1/, "").trim();
  // Remove all N/A occurrences
  text = text.replace(/N\/A/gi, " ").replace(/\s+/g, " ").trim();

  // The last number should be the fee (2,500 or 2500)
  // Match any number with optional comma-thousands separator
  const allNumbers = text.match(/\d{1,3}(?:,\d{3})+|\d+/g);
  if (!allNumbers || allNumbers.length === 0) return out;
  
  const lastNumber = allNumbers[allNumbers.length - 1];
  const amount = normalizeAmount(lastNumber);

  // Everything before the last number is the description
  const lastNumIndex = text.lastIndexOf(lastNumber);
  let description = text.slice(0, lastNumIndex).trim();
  
  if (!description) {
    description = "Irrespective of the loan amount";
  }
