...
export type TariffRow = {
  bank: string;
  product: "Home Loan" | "Education Loan" | "Solar Loan";
    ...
    if (/\bLKR\b.*\bto\b.*\bLKR\b/i.test(cur.text)) {
      ...
      if (norm && nxt && /Small\s*Asset\s*\/?\s*Education\s*\/?\s*Travel\s*\/?\s*Solar\s*\/?\s*Women/i.test(nxt.text)) {
        ...
        rows.push({
          ...
          updatedAt,
          source: SRC,
          notes: "Small Asset/Education/Travel/Solar/Women - same tariff",
        });
        
        // Solar Loan (same tariff as Education)
        rows.push({
          bank: BANK,
          product: "Solar Loan",
          feeCategory: "Processing Fee",
          description: "Processing Fee - Solar Loan",
          amount: norm, // "LKR 1,250 - LKR 7,000"
          updatedAt,
          source: SRC,
          notes: "Small Asset/Education/Travel/Solar/Women - same tariff",
