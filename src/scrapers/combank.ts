...
/** Pensioner Loans inside "All Other Advances" */
async function scrapePensionerFromAllOtherAdvances(block: Locator, nowISO: string): Promise<RateRow[]> {
  const out: RateRow[] = [];
  ...
  for (const table of tables) {
    ...
    for (const tr of trs) {
      ...
      // Section heading - simple match for "pensioner" text
      if (cells.length === 1 && /pensioner.*loan/i.test(rowText)) {
        inPensioner = true;
        continue;
      }

      // Also check if it's a header row (might have blank cells)
      if (/pensioner.*loan/i.test(rowText) && cells.length <= 2) {
        inPensioner = true;
        continue;
      }

      // Exit section if another major heading appears (not subheadings like "Fixed Rates")
      if (cells.length === 1 && inPensioner) {
        // Check if it's a real section change (contains loan/lease keywords)
        const isMajorSection = /(loan|advance|overdraft|deposit|financing|lease)/i.test(rowText);
        const isNotPensioner = !/pensioner/i.test(rowText);
        const isNotRatesSubheading = !/\brates?\b/i.test(rowText) || /hybrid\s+rates/i.test(rowText);
        
        if (isMajorSection && isNotPensioner && isNotRatesSubheading) {
          inPensioner = false;
          continue;
        }
        // Otherwise it's a subheading, skip it
        continue;
      }

      // Data rows: first cell is tenure, last cell is rate
      if (!inPensioner || cells.length < 2) continue;
      ...
      // Skip if it's a header row (Fixed Rates, etc.)
      if (/fixed\s*rates?/i.test(desc)) continue;

      // Parse tenure - handle "1 to 2 Years" format
      let tenureLabel = desc.replace(/^\s*-\s*/, "").trim();
      let years = expandTenureYears(tenureLabel);

      // Manual parsing for "X to Y Years" format
      if (!years.length) {
        const m = tenureLabel.match(/(\d{1,2})\s*to\s*(\d{1,2})\s*(?:year|yr)s?/i);
        if (m) {
          const a = parseInt(m[1], 10);
          const b = parseInt(m[2], 10);
          years = Array.from({ length: b - a + 1 }, (_, i) => a + i);
        }
      }

      // Fallback: single number range "8 to 10 Years"
      if (!years.length) {
        const m2 = tenureLabel.match(/(\d{1,2})\s*to\s*(\d{1,2})/i);
        if (m2) {
        ...
        ...fanOutByYears<RateRow>(
          {
            ...
            source: URL,
            updatedAt: nowISO,
            notes: "Pensioners' Loans - Fixed Rates",
          },
          years
        )
      );
...
/** Solar Loans (Green Loans/Leases) inside "All Other Advances" */
async function scrapeSolarFromAllOtherAdvances(block: Locator, nowISO: string): Promise<RateRow[]> {
  const out: RateRow[] = [];
  ...
  for (const table of tables) {
    ...
    for (const tr of trs) {
      ...
      // Section heading - "Green Loans / Leases"
      if (cells.length === 1 && /green\s+loans?.*leases?/.test(rowText)) {
        inGreen = true;
        continue;
      }

      // Exit section if another heading appears
      if (cells.length === 1 && inGreen && !/green/.test(rowText)) {
        inGreen = false;
        continue;
      }

      // Data rows: first cell is tenure, last cell is rate
      if (!inGreen || cells.length < 2) continue;
      ...
      // Handle ranges like "6 & 7 Years"
      const andMatch = tenureLabel.match(/\b(\d{1,2})\s*&\s*(\d{1,2})\s*(?:year|yr)s?\b/i);
      if (andMatch) {
        years = [parseInt(andMatch[1], 10), parseInt(andMatch[2], 10)];
      } else {
        years = expandTenureYears(tenureLabel);
      }
...
export async function scrapeCombank(
  ...
  try {
    ...
    // 4) ALL OTHER ADVANCES → Education Loans (two sub-groups)
    {
      ...
      rows.push(...(await scrapePensionerFromAllOtherAdvances(block, now)));
      rows.push(...(await scrapeSolarFromAllOtherAdvances(block, now)));
    }
  } finally {
    await browser.close();
  }
