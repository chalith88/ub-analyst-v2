...
/** Pensioner Loan (Sampath Samachara) - 12.00% flat rate */
function extractPensionerLoan(text: string): RateRow[] {
  const out: RateRow[] = [];
  const re = /Sampath\s+Samachara.*?Loan.*?[Ss]cheme.*?for\s*pensioners/i;
  const section = sectionAfter(text, re, 300);
  if (!section) return out;

  const rate = percentAfter(section, 0, 200);
  if (rate) {
    out.push({
      bank: "Sampath Bank",
      ...
      source: SOURCE,
      updatedAt: nowISO(),
      notes: "Sampath Samachara - Loan Scheme for pensioners",
    });
  }
  return out;
}

/** Solar Loan (Green Energy Scheme) - tiered by loan amount */
function extractSolarLoan(text: string): RateRow[] {
  const out: RateRow[] = [];
  const re = /Green\s+Energy\s+Scheme/i;
  const section = sectionAfter(text, re, 400);
  if (!section) return out;

  // Match patterns like "Upto LKR 5.0Mn – 10.50%" and "Above LKR 5.0Mn – 11.00%"
  const uptoMatch = section.match(/Upto\s+LKR\s+[\d.]+\s*Mn.*?(\d{1,2}[.,]\d{2})\s*%/i);
  const aboveMatch = section.match(/Above\s+LKR\s+[\d.]+\s*Mn.*?(\d{1,2}[.,]\d{2})\s*%/i);

  if (uptoMatch) {
    const rate = uptoMatch[1].replace(",", ".") + "%";
    out.push({
      bank: "Sampath Bank",
      product: "Solar Loan",
      type: "Floating",
      ...
      source: SOURCE,
      updatedAt: nowISO(),
      notes: "Green Energy Scheme - Upto LKR 5.0Mn",
    });
  }

  if (aboveMatch) {
    const rate = aboveMatch[1].replace(",", ".") + "%";
    out.push({
      bank: "Sampath Bank",
      product: "Solar Loan",
      type: "Floating",
      ...
      source: SOURCE,
      updatedAt: nowISO(),
      notes: "Green Energy Scheme - Above LKR 5.0Mn",
    });
  }
...
function extractRatesFromText(text: string): RateRow[] {
  ...
  return [
    ...
    ...extractSalariedEmployees(t),
    ...extractPensionerLoan(t),
    ...extractSolarLoan(t),
  ];
}
