/**
 * NSB Market Share OCR Scraper
 * Extracts total gross loans from NSB quarterly reports using OCR
 * 
 * Report URL: https://www.nsb.lk/financial-reports/
 * Target Data: Gross Loans and Advances
 * 
...
async function scrape(): Promise<MarketShareResult | null> {
  try {
    console.log("📊 Scraping NSB Market Share...");
    // Note: NSB URL would need to be determined from their website
    const url = "https://www.nsb.lk/wp-content/uploads/2025/11/Interim-Financials-30.09.2025-Web-version.pdf";
    const pdfPath = "./tmp/nsb-q3-2025.pdf";
    ensureDir("./tmp");
    ...
    ensureDir("./output");
    fs.writeFileSync("./output/nsb-market-share-ocr-lines.txt", lines.map((l, i) => `[${i}] ${l}`).join("\n"));
    ...
    for (const line of lines) {
      ...
      // NSB has "Sub Total" for product-wise total (line 395)
      if ((lower.includes("sub") && lower.includes("total")) || lower.includes("subtotal")) {
        ...
        if (numbers && numbers.length >= 2) {
          ...
          if (current > 530000000 && current < 600000000) {
            total = current;
            prevTotal = previous;
    ...
    const result: MarketShareResult = {
      ...
      source: "NSB Q3 2025 Interim Financial Statements",
    ...
    fs.writeFileSync("./output/nsb-market-share-ocr.json", JSON.stringify(result, null, 2));
...
export { scrape as scrapeNSBMarketShare };
